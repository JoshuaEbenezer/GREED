...
scores_df = pd.read_csv('../LIVE-HFR/subjective_scores/subject_MOS.csv')
print(len(scores_df))
...
#    feature_folder= "/home/ubuntu/bitstream_mode3_p1204_3/features/p1204_etri_features"
    ...
    feature_folder= './ythfr_temporal_brisque_features'
...
#        else:
        featfile_name = vid+'.z'
        feat_file = list(load(os.path.join(feature_folder,featfile_name)))
        ms_spatial_brisque = np.average(feat_file[0],axis=1)
        ms_temp_brisque = np.average(feat_file[1],axis=2)
#        print(feat_file)
        del feat_file
#        feat_file2 = load(os.path.join(feature_folder2,featfile_name))
        flist_arr = [ms_spatial_brisque.flatten(),ms_temp_brisque.flatten()]
        feature = np.concatenate(flist_arr)
