...
def sts_fromfilename(i,filenames,results_folder):
    ...
    image_outname = './images/sts_arr'+os.path.splitext(name)[0]+'.png'
    if(os.path.exists(image_outname)):
        return
...
#        print(framenum)
        ...
        if (i>=st_time_length): 
            ...
            dump(sts_arr,'./sts_arr_data/'+os.path.splitext(name)[0]+'.z')
            plt.figure()
            plt.clf()
            plt.hist(np.asarray(sts).flatten(),bins=1000,histtype='step',density=True)
            plt.savefig('./images/sts_arr'+os.path.splitext(name)[0]+'.png')
            plt.close()
            return 
...
#            sts_grad = np.reshape(sts_grad,(r1*st_time_length,r2*st_time_length))
            dsts_arr = unblockshaped(np.reshape(dsts,(-1,st_time_length,st_time_length)),r1*st_time_length,r2*st_time_length)
            dsts_grad = unblockshaped(np.reshape(dsts_grad,(-1,st_time_length,st_time_length)),r1*st_time_length,r2*st_time_length)
...
#            for i in range(len(files)))
    for i in range(len(files)):
        sts_fromfilename(i,files,outfolder)
