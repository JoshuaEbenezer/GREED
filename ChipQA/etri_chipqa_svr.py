...
#    feature_folder= "/home/ubuntu/bitstream_mode3_p1204_3/features/p1204_etri_features"
    ...
    feature_zfile = load('../chipqa_etri_combined.z')
    zfile_names = feature_zfile['name']
    features = feature_zfile['features']
    train_names = []
    val_names = [] 
...
#        else:
        featfile_name = vid+'.z'
        score = scores[i]
        feat_index = zfile_names.index(featfile_name)

        feature = features[feat_index,:]
...
srcc_csv.to_csv('results/etri_chipqa_srcc_lcc_rmse_list.csv')
